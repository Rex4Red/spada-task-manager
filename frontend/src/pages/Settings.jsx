...
const Settings = () => {
    ...
    // Poll WhatsApp status every 3 seconds when QR is showing
    useEffect(() => {
    ...
    const fetchSettings = async () => {
        try {
            ...
            // Pre-fill WhatsApp phone if exists
            if (data.whatsappConfig) {
                setWhatsappPhone(data.whatsappConfig.phoneNumber || '');
                setIsWhatsappActive(data.whatsappConfig.isActive);
            }
        } catch (error) {
    ...
    const handleTestWhatsapp = async () => {
        ...
        try {
            ...
            alert('Test notification sent to WhatsApp!');
        } catch (error) {
            console.error('WhatsApp test failed:', error);
            const msg = error.response?.data?.message || 'Failed to send WhatsApp test notification.';
            alert(msg);
        } finally {
            ...
            <div className="flex-1 w-full max-w-[1200px] mx-auto px-4 md:px-8 py-6 flex flex-col gap-8">
                ...
                <div className="flex flex-col gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-white text-xl font-bold">WhatsApp Notifications</h2>
                    </div>
                    ...
                    <div className="bg-card-dark rounded-xl border border-[#283039] p-6 flex flex-col gap-6">
                        ...
                        <div className="flex flex-col gap-4">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    <div className={`w-3 h-3 rounded-full ${waStatus === 'connected' ? 'bg-green-500' : waStatus === 'qr' ? 'bg-yellow-500 animate-pulse' : 'bg-red-500'}`}></div>
                                    <span className="text-white font-medium">
                                        {waStatus === 'connected' ? '🟢 Connected' : waStatus === 'qr' ? '🟡 Scan QR Code' : waStatus === 'connecting' ? '🟡 Connecting...' : '🔴 Disconnected'}
                                    </span>
                                </div>
                            ...
                            {waStatus === 'qr' && waQrCode && (
                                <div className="flex flex-col items-center gap-3 py-4">
                                    <p className="text-[#9dabb9] text-sm text-center">Scan this QR code with WhatsApp on your phone:</p>
                                    <div className="bg-white p-3 rounded-xl">
                                        <img src={waQrCode} alt="WhatsApp QR Code" className="w-64 h-64" />
                                    </div>
                                    <p className="text-xs text-[#5e6a75] text-center">Open WhatsApp → Settings → Linked Devices → Link a Device</p>
                                </div>
                            )}

                            {waStatus === 'connecting' && (
                                <div className="flex items-center justify-center py-4">
                                    <div className="animate-spin w-8 h-8 border-4 border-[#25D366] border-t-transparent rounded-full"></div>
                                    <span className="ml-3 text-[#9dabb9]">Connecting to WhatsApp...</span>
                                </div>
                            )}
                        </div>

                        {/* Divider */}
                        {waStatus === 'connected' && <div className="h-px bg-[#283039] w-full"></div>}

                        <p className="text-[#9dabb9] text-sm">
                            Get notified about upcoming deadlines directly to your WhatsApp.
                            <br />
                            Enter your phone number with country code (e.g. <b>6281234567890</b> for Indonesia).
                        </p>

                        <div className="flex flex-col gap-4 max-w-xl">
                            <div className="flex flex-col gap-2">
                                <label className="text-white text-sm font-medium">WhatsApp Phone Number</label>
                                <input
                                    type="text"
                                    value={whatsappPhone}
                                    onChange={(e) => setWhatsappPhone(e.target.value)}
                                    placeholder="e.g. 6281234567890"
                                    className="bg-[#101922] border border-[#3b4754] rounded-lg px-4 py-3 text-white focus:outline-none focus:border-[#25D366] transition-colors"
                                />
                                <p className="text-xs text-[#5e6a75]">Include country code without + symbol (e.g. 62 for Indonesia)</p>
                            </div>

                            <div className="flex items-center gap-3">
                                <button
                                    onClick={() => setIsWhatsappActive(!isWhatsappActive)}
                                    className={`w-12 h-6 rounded-full relative transition-colors ${isWhatsappActive ? 'bg-[#25D366]' : 'bg-[#283039]'}`}
                                >
                                    <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${isWhatsappActive ? 'left-7' : 'left-1'}`}></div>
                                </button>
                                <span className="text-white text-sm">Enable WhatsApp Notifications</span>
                            </div>
                            ...
                            <div className="flex items-center gap-3 mt-2">
                                <button
                                    ...
                                    className="px-6 py-2.5 bg-[#25D366] hover:bg-[#20BD5A] text-white font-bold rounded-lg transition-colors disabled:opacity-70"
                                >
                                    {savingWhatsapp ? 'Saving...' : 'Save WhatsApp'}
                                </button>
                                <button
                                    onClick={handleTestWhatsapp}
                                    disabled={testingWhatsapp || !whatsappPhone || waStatus !== 'connected'}
                                    className="px-6 py-2.5 bg-[#283039] text-white font-medium rounded-lg hover:bg-[#323b46] border border-[#3b4754] transition-colors disabled:opacity-50"
                                >
